...
public class Reflections {
	...
	public static <T> List<Class<?>> getTypeArguments(Class<T> cls, Class<? extends T> cls2) {
		...
		Type typeVar = cls2; // Use Type variable to hold both Class and ParameterizedType
		while (true) {
			i = 0;
			if (getClass(typeVar).equals(cls)) {
				break;
			}
			if (typeVar instanceof Class) {
				Type genericSuperclass = ((Class<?>) typeVar).getGenericSuperclass();
				if (genericSuperclass instanceof Class) {
					typeVar = genericSuperclass;
				} else {
					typeVar = genericSuperclass;
				}
			} else {
				ParameterizedType parameterizedType = typeVar instanceof ParameterizedType ? (ParameterizedType) typeVar : null;
				...
				if (!cls3.equals(cls)) {
					typeVar = cls3.getGenericSuperclass();
				}
			}
		}
		if (typeVar instanceof Class) {
			actualTypeArguments = ((Class<?>) typeVar).getTypeParameters();
		} else {
			if (!(typeVar instanceof ParameterizedType)) return null;
			actualTypeArguments = ((ParameterizedType) typeVar).getActualTypeArguments();
		}
		ArrayList arrayList = new ArrayList();
