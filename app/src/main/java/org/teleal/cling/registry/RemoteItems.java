...
class RemoteItems extends RegistryItems<RemoteDevice, RemoteGENASubscription> {
	...
	public void add(final RemoteDevice remoteDevice) {
		...
		RegistryItem<UDN, RemoteDevice> registryItem = new RegistryItem<>(remoteDevice.getIdentity().getUdn(), remoteDevice, remoteDevice.getIdentity().getMaxAgeSeconds().intValue());
	...
	boolean update(RemoteDeviceIdentity remoteDeviceIdentity) {
		...
		final RegistryItem<UDN, RemoteDevice> registryItem = new RegistryItem<>(root.getIdentity().getUdn(), root, remoteDeviceIdentity.getMaxAgeSeconds().intValue());
	...
	void maintain() {
		...
		while (it2.hasNext()) {
			...
			if (registryItem2.getExpirationDetails().hasExpired(true)) {
				hashSet.add((RemoteGENASubscription) registryItem2.getItem());
			}
		}
